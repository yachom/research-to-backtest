...
| ID | 원 마일스톤 | 내용 |
|---|---|---|
| H1 | (신설) | HITL 기반 계층: 산출물 모델(CandidateAnalysis·AnalystView·HumanInvestmentHypothesis·StrategyReview·BacktestInterpretation·AuthoredContent), 파이프라인 상태 머신 12종 + 승인 게이트, run_id 산출물 저장소, 검증 규칙 — Wave 1과 병렬 가능 |
| C1' | M7 개정 | Evidence Store 구축 + **HypothesisCandidateGenerator**(구 가설 생성기 개명·역할 축소): LLM이 CandidateAnalysis·hypothesis_candidates(참고용)만 생성. 프롬프트 버전 파일 + 호출 기록(과제2 증빙) |
| C2' | M8 개정 | **승인된** HumanInvestmentHypothesis → LLM 전략 DSL 초안 → Validator → 사용자 검토·수정(StrategyModification) → 승인(StrategyReview) → 백테스트 전달 |
