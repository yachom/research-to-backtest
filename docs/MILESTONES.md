...
| 웨이브 | 병렬 트랙 | 의존성 |
|---|---|---|
| Wave 1 | **A4**(core/financials — 재무 정규화·시계열) ∥ **A5**(quant/strategy — 전략 DSL) ∥ **B1+B2**(core/xbrl — XBRL 수집·파싱) | 셋 다 상호 독립 (A2·A3 데이터와 A1 코드에만 의존) |
| Wave 2 | **A6**(quant/backtest — A4·A5 계약 의존) ∥ **B3**(정합성 검증 — B2+A4 의존) ∥ **B4**(정정공시 PIT) | Wave 1 병합 후 |
| Wave 3 | **C1·C2**(LLM 리서치·NL→DSL — 구조는 키 없이, live는 OpenRouter 키 확보 후) → **C3**(마무리) | Wave 2 병합 후 |
